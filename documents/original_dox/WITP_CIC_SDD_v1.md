...
**Version:** 1.0 (final, consolidated) · **Target platform:** macOS · **Stack:** Python 3.11+, PySide6 (Qt), SQLite
...
```
Enums — `belief_status`: OPERATIONAL|DAMAGED|SUNK_UNCONFIRMED|SUNK_PROBABLE|LOST_CONTACT. `alert_type`: MOVING_TAIL|COURSE_CHANGE|RADIO_SILENCE|BDA_CONFLICT|UNCONFIRMED_KILL|IDENT_MISMATCH|CONCENTRATION|PORT_CONCENTRATION|CONVERGENCE|INVASION_WARNING|SORTIE|ROUTE_ASCERTAINED|HABIT_PREDICTION|UNIT_COMMITTED|REPAIR_STALLED|CORRECTION. `severity`: INFO|WARN|CRIT.
...
**S8.2 belief/corrections:** sunk claim → SUNK_UNCONFIRMED conf bda_skepticism (1.0 own fact); +0.05/quiet turn cap 0.90; ≥0.75 → SUNK_PROBABLE; re-sighting after sunk → DAMAGED + BDA_CONFLICT (CRIT) + CORRECTION; own ADMIT/OWNSINK/SINK → 1.0; enemy SINK ledger → 0.8; class conflict same hex±2 same turn → alt_names + IDENT_MISMATCH (WARN) if warship class.
...
## S9. UI (PySide6, macOS)
S9.1 Splash 2 s. S9.2 Wizard (if no config): side combo (note "Allied SIGINT treated as certain"); export dir (auto-scan `~/Documents/WITP_AE_Exports`); validation = 4 filename patterns + ≥1 common YYMMDD, failure shows missing list + guidance; turn duration 12/24. S9.3 Main: min 1280×800; dark palette bg `#0d1117` panel `#161b22`; CERTAIN `#3FB950` CLAIM `#D29922` CRIT `#F85149` INFO `#58A6FF` LOST `#8B949E`; Allied `#4D9DE0` Japanese `#E07B39`. Header: side badge, turn label + ◀ ▶, PROCESS TURN, ⚙. Left AlertFeed (filters, unread dots). Center QTabWidget: Tab1 "Intel Summary" = nested tabs [HVT | Concentrations | Ports | Routes | Corrections] QTableView+QSortFilterProxyModel; Tab2 "SIGINT Map" = QGraphicsView, `assets/map.jpg` or grey grid, two-click calibration stored in map_cal, layer checkboxes (radio, sightings, HVT, concentration circles, routes), click → dossier. Right DossierPanel (QCompleter, report, Export/Copy). Bottom EventLog (source chips). Status bar (db path, last parse, obs, skipped clickable, watcher LED). S9.4 Behaviors: alert click → tab+row+dossier; row click → dossier; scrubber re-queries as-of-T; QFileSystemWatcher + auto_process → toast; parse errors → amber banner. S9.5 macOS: global menu bar, `Cmd+,` settings, `Cmd+P` process, `Cmd+F` dossier, `Cmd+1/2` tabs, `←/→` scrub; SF Mono for hexes/logs; density rowHeight S22/M28/L34. S9.6 Packaging: `pip install pyside6 pytest pyinstaller`; PyInstaller `--windowed --name "WITP CIC"`; `create-dmg`; CI with `QT_QPA_PLATFORM=offscreen`.
